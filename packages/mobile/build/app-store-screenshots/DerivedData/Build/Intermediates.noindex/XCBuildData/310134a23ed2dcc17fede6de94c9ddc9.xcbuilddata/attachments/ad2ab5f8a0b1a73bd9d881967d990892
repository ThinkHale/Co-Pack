...
STAMP_FILE="$DERIVED_FILE_DIR/expo-dsym-resolve-ExpoFileSystem-$CONFIGURATION.stamp"

# Find dSYMs inside the xcframework (all slices)
XCFW_PATH="$PODS_ROOT/ExpoFileSystem/ExpoFileSystem.xcframework"

if [ -d "$XCFW_PATH" ]; then
  FOUND_DSYM=0
  for DSYM_DIR in "$XCFW_PATH"/*/dSYMs; do
    if [ -d "$DSYM_DIR/ExpoFileSystem.framework.dSYM" ]; then
      ...
      "$NODE_BINARY" "$PODS_ROOT/../../../../node_modules/expo-modules-autolinking/scripts/ios/resolve-dsym-sourcemaps.js" \
        ...
        -m "ExpoFileSystem" \
        -n "expo-file-system" \
        -r "$PODS_ROOT/../../node_modules/expo-file-system"
    fi
  done
fi
