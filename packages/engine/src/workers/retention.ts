import { GameState, GameEvent, Worker, Line } from '../types';
 ...
 */
export function quitProbability(worker: Worker): number {
...
export function flightRisk(worker: Worker): 'low' | 'watch' | 'high' {
  const p = quitProbability(worker);
